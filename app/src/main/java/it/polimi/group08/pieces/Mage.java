 ...
 * Mage has four boolean attribute(freezeEnable, healEnable, reviveEnable, teleportEnable) to record different spells are used or not.
 * Mage has four methods to cast spells, if it casts spells successfully, return true, otherwise return false
 * getSpellsUnused is to return a string recording unused spells to board
 * setSpellsUnused is to set unused spells into Mage class, when the game is initialized
 */
...
public class Mage extends Piece {
    ...
    public boolean revive(Piece p,int x, int y){
        if(reviveEnable) {
            reviveEnable = false;
            p.revived(x,y);
            return true;
